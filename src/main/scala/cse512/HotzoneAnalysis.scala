...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    // find total no of points contained inside rectangle
    var temp = spark.sql("select rectangle,count(point) as point from joinResult group by rectangle order by rectangle")
    temp.createOrReplaceTempView("result")
    val result = spark.sql("select count(*) from result")

    // for getting one output part file
    temp = spark.sql("select * from result limit " + result.first().getLong(0).toString())
    temp.show()
    return temp
